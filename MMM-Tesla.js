...
Module.register('MMM-Tesla', {
  ...
  start: function () {
    ...
    for (const identifier in DataItemProvider.providers) {
  ...
  getDom: function () {
    ...
    for (const index in this.config.items) {
      const dataItem = this.config.items[index]

      if (!Object.hasOwn(this.providers, dataItem)) {
      ...
      const icon = this.providers[dataItem].icon
      const field = this.providers[dataItem].field
      const value = this.providers[dataItem].value
  ...
  socketNotificationReceived: function (notification, payload) {
    if (notification === 'STARTED') {
      ...
      for (const identifier in this.providers) {
  ...
  convertDistance: function (valueMiles) {
    if (this.config.unitDistance === 'km') {
      const valueKm = valueMiles * 1.60934
